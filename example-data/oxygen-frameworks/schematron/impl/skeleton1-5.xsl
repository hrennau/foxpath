...
<!-- 
   ...
   Version 2005-08-25 (Syncro Soft SRL)
   	* add generatedStylesheetVersion parameter.
   Version 2006-06-12 (george bina)
    * Replace not(../sch:rule) with
      not(generate-id(..)=generate-id(parent::*[self::sch:rule])
   Version 2006-06-29 (george bina)
    * Handle the XSL namespace.
-->
...
<xsl:param name="generatedStylesheetVersion">1.0</xsl:param><!-- version for the generated stylesheet -->
<!-- SCHEMA -->
<xsl:template match="sch:schema | schema">
	<axsl:stylesheet>
		<xsl:for-each select="sch:ns | ns">
		  <xsl:choose>
		    <xsl:when test="function-available('exsl:node-set')">
		      ...
		      <xsl:copy-of select="exsl:node-set($ns-dummy-elements)/*/namespace::*"/>
		    </xsl:when>
		    ...
		    <xsl:when test="system-property('xsl:version')='2.0'">
		      ...
		      <xsl:copy-of select="$ns-dummy-elements/*/namespace::*"/>
		    </xsl:when>
		    <xsl:when test="@uri = 'http://www.w3.org/1999/XSL/Transform'">
		      <xsl:message terminate="yes">
		        <xsl:text>Using the XSLT namespace in </xsl:text>
	  ...
	  <xsl:attribute name="version"><xsl:value-of select="$generatedStylesheetVersion"/></xsl:attribute>
	...
	<xsl:template  match="sch:key | key " mode="do-keys" >
               ...
               <xsl:if test="not(@match) and not(generate-id(..)=generate-id(parent::*[self::sch:rule]))">
	...
	<xsl:template match="sch:rule[not(@abstract='true')] | rule[not(@abstract='true')]">
		...
		<axsl:template match="{@context}" priority="{100 + count(../*) - count(preceding-sibling::*)}" mode="M{count(../preceding-sibling::*)}">
			<xsl:copy-of select="@*[namespace-uri() != '']"/>
