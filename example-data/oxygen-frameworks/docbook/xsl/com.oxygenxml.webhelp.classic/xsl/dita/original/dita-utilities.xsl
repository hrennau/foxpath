...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="*" mode="get-render-direction">
    <xsl:param name="lang">
      ...
      <xsl:choose>
          <xsl:when test="starts-with($lang, 'ar')">rtl</xsl:when>
          <xsl:when test="starts-with($lang, 'he')">rtl</xsl:when>
          <xsl:when test="starts-with($lang, 'ur')">rtl</xsl:when>
          <xsl:when test="starts-with($lang, 'fa')">rtl</xsl:when>
          <xsl:when test="starts-with($lang, 'iw')">rtl</xsl:when>
          <xsl:when test="starts-with($lang, 'ota')">rtl</xsl:when>
  ...
  <xsl:template name="getString">
    ...
    <xsl:param name="ancestorlang">
      ...
      <xsl:otherwise>
          ...
          <xsl:variable name="stringFileListLocal">
            <xsl:choose>
              <xsl:when test="string-length($stringFileList) > 0">
                <xsl:value-of 
                      select="concat('../../../oxygen-webhelp/resources/localization/', 
                                            $stringFileList)"/>
              </xsl:when>
              <xsl:when test="count(document('../../../../../xsl/common/allstrings.xml')/*) > 0">
                <xsl:value-of select="concat('../../../../../xsl/common/',
                               document('../../../../../xsl/common/allstrings.xml')/allstrings/stringfile[1])"/>
              </xsl:when>
            </xsl:choose>
          </xsl:variable>
        <xsl:if test="string-length($stringFileListLocal) > 0">
          <xsl:variable name="stringfile"
                  select="document($stringFileListLocal)/*/lang[@xml:lang=$ancestorlang]/@filename"/>
            <!-- Get the translated string -->
            ...
            <xsl:choose>
                ...
                <xsl:when test="$ancestorlang!=$DEFAULTLANG">
                  ...
                  <xsl:variable name="backupstringfile"
                      select="document($stringFileListLocal)/*/lang[@xml:lang=$DEFAULTLANG]/@filename"/>
                  <xsl:variable name="str-default"
