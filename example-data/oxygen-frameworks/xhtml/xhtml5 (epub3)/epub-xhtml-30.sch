<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="idrefs-aria-describedby">
        <rule context="*[@aria-describedby]">
            <assert test="every $idref in tokenize(@aria-describedby,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@aria-describedby"/> attribute must refer to elements in the same
    ...
    <pattern id="idrefs-output-for">
        <rule context="h:output[@for]">
            <assert test="every $idref in tokenize(@for,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@for"/> attribute must refer to elements in the same
                document (target ID missing)</assert>
        </rule>
    </pattern>
    ...
    <pattern id="idrefs-aria-flowto">
        <rule context="*[@aria-flowto]">
            <assert test="every $idref in tokenize(@aria-flowto,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@aria-flowto"/> attribute must refer to elements in the same
                document (target ID missing)</assert>
        </rule>
    </pattern>
    ...
    <pattern id="idrefs-aria-labelledby">
        <rule context="*[@aria-labelledby]">
            <assert test="every $idref in tokenize(@aria-labelledby,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@aria-labelledby"/> attribute must refer to elements in the same
                document (target ID missing)</assert>
        </rule>
    </pattern>
    ...
    <pattern id="idrefs-aria-owns">
        <rule context="*[@aria-owns]">
            <assert test="every $idref in tokenize(@aria-owns,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@aria-owns"/> attribute must refer to elements in the same
                document (target ID missing)</assert>
        </rule>
    </pattern>
    ...
    <pattern id="idrefs-aria-controls">
        <rule context="*[@aria-controls]">
            <assert test="every $idref in tokenize(@aria-controls,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))">The <name path="@aria-controls"/> attribute must refer to elements in the same
                document (target ID missing)</assert>
        </rule>
    </pattern>
    ...
    <pattern id="idref-contextmenu">
        <rule context="h:*[@contextmenu]">
            <assert test="//h:menu[@id = current()/@contextmenu]">The <name path="@contextmenu"/> attribute does not refer to an allowed target element
                (expecting: <value-of select="replace('h:menu','h:','')"/>).</assert>
        </rule>
    </pattern>
    ...
    <pattern id="style-scoped">
        <rule context="h:style[ancestor::h:body]">
            ...
            <assert test="every $elem in preceding-sibling::* satisfies (local-name($elem) eq 'style') or (local-name($elem) eq 'figcaption') ">The scoped style element must occur before any other flow content other than other
