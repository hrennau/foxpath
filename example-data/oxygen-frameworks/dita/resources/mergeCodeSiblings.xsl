...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*:p[not(contains(@class, 'MsoListParagraph'))]
        [contains(@id, 'docs-internal-guid') or 
            contains(@class, 'Paragraph') or 
                preceding-sibling::*:br[contains(@id, 'docs-internal-guid')] or 
                    preceding-sibling::*:p[contains(@id, 'docs-internal-guid')]] " mode="merge">
        <xsl:call-template name="mergeCourierNewSpansToCodeElements"/>
    </xsl:template>   
