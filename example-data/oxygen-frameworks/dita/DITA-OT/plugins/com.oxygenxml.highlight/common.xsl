        ...
        <xsl:choose>
          <xsl:when test="$config//*[@id=$type]">
              ...
              <xsl:apply-templates 
                  ...
                  use-when="function-available('ConnectorSaxonEE:highlight') and (version:getProductVersion() ne '9.1.0.8')"/>
              <xsl:apply-templates 
                  select="ConnectorSaxonB:highlight($type, $content, base-uri($config))" mode="xslthl" 
                  use-when="function-available('ConnectorSaxonB:highlight') and (version:getProductVersion() eq '9.1.0.8')"/>
              <xsl:copy-of select="$content" 
                use-when="not(function-available('ConnectorSaxonEE:highlight')) and not(function-available('ConnectorSaxonB:highlight'))"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:copy-of select="$content"/>
          </xsl:otherwise>
