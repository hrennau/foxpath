...
JATSKit REVISIONS 2017

Refactored floats handling to expose float control via switch
Stylesheet parameter $suppress-floats='yes' will turn off floating
behavior globally. A string is expected.

Also removed traces of fo:wrapper, which was breaking FOP.

 = = = = =

First NISO JATS version (wap) v1.0 (2012-06-28)
...
<!-- These affect the operation of the stylesheet as a whole. They
  ...
  <!-- Will be treated as a string; 'yes' turns on. -->
  <xsl:param name="suppress-floating" select="'no'"/>
  
  <!-- $allow-floats is a Boolean value. -->
  <xsl:variable name="allow-floats" select="not($suppress-floating='yes')"/>
    ...
    <xsl:with-param name="contents">
      <fo:block-container xsl:use-attribute-sets="media-object">
        <xsl:apply-templates select="@orientation"/>
          <fo:block start-indent="0pc" line-stacking-strategy="max-height">
...
<xsl:template match="boxed-text">
  <xsl:call-template name="set-float">
    <xsl:with-param name="contents">
      <fo:block-container xsl:use-attribute-sets="boxed-text"  start-indent="0pc">
        <xsl:apply-templates select="@orientation"/>
          <xsl:apply-templates select="." mode="label"/>
          <xsl:apply-templates/>
      </fo:block-container>
...
<xsl:template match="fig | fig-group">
  <xsl:call-template name="set-float">
    <xsl:with-param name="contents">
      <fo:block-container xsl:use-attribute-sets="fig-box"  start-indent="0pc">
        <xsl:apply-templates select="@orientation"/>
          <xsl:apply-templates select="." mode="label"/>
          <xsl:apply-templates/>
...
<xsl:template match="supplementary-material">
  <xsl:call-template name="set-float">
    <xsl:with-param name="contents">
      <fo:block-container xsl:use-attribute-sets="supplementary"  start-indent="0pc">
        <xsl:apply-templates select="@orientation"/>
          <xsl:apply-templates select="." mode="label"/>
          <xsl:apply-templates/>
      </fo:block-container>
...
<xsl:template match="table-wrap | table-wrap-group">
  <xsl:call-template name="set-float">
    <xsl:with-param name="contents">
      <fo:block-container xsl:use-attribute-sets="table-box"  start-indent="0pc">
...
<xsl:template match="preformat">
  ...
  <xsl:call-template name="set-float">
    <xsl:with-param name="float-okay"
      select="$allow-floats and
...
<!--<xsl:include href="jats-preview-xslt/xslt/main/xhtml-tables-fo.xsl"/>-->
...
<xsl:template match="*" mode="floats">
  <!-- floats are rendered in place unless they are both 
       ...
       do not have @position != (not equal) "float", and
       $allow-float is true(), in which case
       they have been floated to near the point of the xref -->
  <xsl:variable name="xrefs" select="key('xref-by-rid',@id)"/>
  <xsl:choose>
     <xsl:when test="boolean($xrefs) and not(@position != 'float') and $allow-floats"/>
     <xsl:otherwise>
       <xsl:apply-templates select=".">
         <xsl:with-param name="float-okay" select="false()"/>
       </xsl:apply-templates>
     </xsl:otherwise>
  </xsl:choose>
...
<xsl:template name="set-float">
  <!-- A float may be prohibited by passing $float-okay as false() -->
  <xsl:param name="float-okay" select="$allow-floats"/>
  <xsl:param name="contents">
    <xsl:apply-templates/>
  </xsl:param>
  ...
  <!-- assuming $allow-floats is true(), the test respects 
