...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" queryBinding="xslt2" 
    ...
    <pattern>
        ...
        <rule context="sch:schema">
            <assert test="not(namespace::sqf) or @queryBinding = 'xslt2' or @queryBinding = 'xslt3'" sqf:fix="setQueryBinding20 setQueryBinding30"> Schematron Quick Fixes are only available within Schematron schemas based on XSLT 2.0/3.0.</assert>
    ...
    <pattern>
        ...
        <rule context="sch:assert[@sqf:fix] | sch:report[@sqf:fix]">
            ...
            <report test="count($notAvailableFixes) gt 0" 
                sqf:fix="deleteRef createLocal createGlobal">The fix(es) <value-of select="string-join($notAvailableFixes, ', ')"/> are not available in this rule.</report>
    ...
    <pattern>
        ...
        <rule context="sch:assert[@sqf:default-fix] | sch:report[@sqf:default-fix]">
            ...
            <let name="fixes" value="tokenize(@sqf:fix, '\s')"/>
            <let name="availableFixIds" value="sqf:getAvailableFixOrGroups(ancestor::sch:rule)/@id"/>
            <assert test="$fixes[. = $defaultFix]" 
                sqf:fix="createLocal addReference deleteAttr">The default fix "<value-of select="$defaultFix"/>" is not referred by the sqf:fix attribute.</assert>
    ...
    <pattern>
        ...
        <rule context="sqf:add[@select] | sqf:replace[@select]">
            <report test="* or normalize-space(.) != ''"  sqf:fix="contentOrSelect">If the select attribute is setted the <name/> element should be empty.</report>
    ...
    <pattern>
        ...
        <rule context="sqf:add[@node-type = 'attribute']">
            <assert test="not(@position)"  sqf:fix="set.node-type deletePosition" sqf:default-fix="deletePosition">If the node-type attribute has the value "attribute" the position attribute should not be set.</assert>
        ...
        <rule context="sqf:add[@node-type != 'comment'] | sqf:replace[@node-type != 'comment']" role="error">
            <assert test="@target"  sqf:fix="set.node-type.comment set.node-type.delete addTarget">The attribute target has to be set if the node-type attribute has not the value "comment".</assert>
        ...
        <rule context="sqf:add[@target] | sqf:replace[@target]" role="fatal">
            <assert test="@node-type"  sqf:fix="set.node-type deleteTarget">The attribute node-type is required if the target attribute has been set.</assert>
    ...
    <pattern>
        ...
        <rule context="sqf:call-fix">
            ...
            <assert test="$ref = $availableFixIds"  sqf:fix="createLocal createGlobal">The QuickFix with the id <value-of select="$ref"/> is not available in this rule.</assert>
            <report test="$ancFix/@id = $ref"  sqf:fix="delete renameFix">The fix should not call its self. It will produce an endless loop.</report>
        ...
        <rule context="sqf:with-param">
            ...
            <assert test="$refFix/sqf:param[@name = $paramName]"  sqf:fix="addParam delete">The called QuickFix has no parameter with the name <value-of select="$paramName"/>.</assert>

            <report test="@select and node()"  sqf:fix="contentOrSelect">If the select attribute is setted the <name/> element should be empty.</report>
    ...
    <pattern>
        ...
        <rule context="sqf:description">
            <report test="count(sqf:title/child::*) = 0 and
                string-join(sqf:title/normalize-space(.), '') = ''" sqf:fix="delete deleteParent" role="warn">The description should have a title.</report>
    ...
    Functions
    -->
    <sch:let name="LOCAL_ONLY" value="'LOCAL_ONLY'"/>
    <sch:let name="GLOBAL_ONLY" value="'GLOBAL_ONLY'"/>
    <sch:let name="GLOBAL_AND_LOCAL" value="'GLOBAL_AND_LOCAL'"/>
