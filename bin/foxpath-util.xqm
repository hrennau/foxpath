...
declare function f:foldText($t as xs:string,
    ...
    return $t ! f:foldTextREC(., $regex, $ops)
};
...
declare function f:foldTextREC($t as xs:string,
    ...
    if (not($regex)) then (
        ...
        substring($t, $width + 1) 
        => f:foldTextREC($regex, $fnUpdOps($options)))
    else
...
:)
(:~
 : Flattens an array, or returns the input item as is, if
 : it is not an array.
 :)
declare function f:flatten($item as item())
        as item()* {
